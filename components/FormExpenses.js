...
class FormExpenses extends Component {
    constructor(props) {
        ...
        const name = ("expenses" in props && props.expenses.name)?props.expenses.name:"";
        const value = ("expenses" in props && props.expenses.value)?props.expenses.value:"";
        const state = ("expenses" in props && props.expenses.state)?(props.expenses.state == 1?true:false):false;
    ...
    submit(){
    	if("onSubmit" in this.props){
    		if("expenses" in this.props){
	    		this.props.onSubmit(Object.assign({}, this.state, {id:this.props.expenses.id}));
    		}else{
            ...
            <View>
                <View style={{paddingBottom:12}}>
                        <View style={styles.inputContainer}>
                            <Text style={styles.labelCircle}>Nombre del egreso</Text>
                            <TextInput
                        ...
                        <View style={styles.inputContainer}>
                            <Text style={styles.labelCircle}>Valor del egreso</Text>
                            <TextInput
                                style={styles.inputCircle}
                        ...
                        <View style={combineStyles([styles.inputContainer, {flexDirection:"row", alignItems:"center"}])}>
                            <Text style={combineStyles([styles.labelCircle, {flex:5}])}>
                                Active esta opción si el egreso ya se hizo efectivo. Esto le indicará a la aplicación
                                que debe incluir el egreso en los cálculos.
                            </Text>
                            <View style={{flex:1}}> 
                                <Switch
